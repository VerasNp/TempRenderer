...
ApplicationConfig
ApplicationConfig::loadFromFile(const std::string &applicationConfigFilePath) {
  const std::unique_ptr<parsers::ParserPort> applicationParser =
      std::make_unique<parsers::TOMLPlusPlusParserAdapter>();
  const parsers::ConfigValue rootApplication =
      applicationParser->parserFile(applicationConfigFilePath);
  ApplicationConfig config;
  try {
    if (const auto windowConfig = rootApplication.get("window")) {
    ...
    LC_LOG(logging::LogLevel::ERROR,
           std::string("Unexpected error while loading application configs '") +
               applicationConfigFilePath + "': " + err.what());
    throw;
  }
